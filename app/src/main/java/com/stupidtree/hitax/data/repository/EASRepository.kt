...
import com.stupidtree.hitax.data.source.web.sysu.SysuWebSource
...
class EASRepository internal constructor(application: Application) {
    ...
    /**
     * 使用统一身份认证（CAS）会话 Cookie 建立教务登录
     * @param cookies WebView 登录后取得的 Cookie
     * @param info 从登录回调中解析出的学生信息（可为空）
     */
    fun loginWithCookies(
        cookies: Map<String, String>,
        info: SysuSession.Info? = null
    ): LiveData<DataState<Boolean>> {
        val cookieText = cookies.entries.joinToString("; ") { "${it.key}=${it.value}" }
        return easService.login(info?.userNumber ?: "", "", cookieText).map {
            if (it.state == DataState.STATE.SUCCESS) {
                it.data?.let { token ->
                    // 用登录回调中的信息补全
                    info?.let { i ->
                        if (!i.userName.isNullOrEmpty()) token.name = i.userName
                        if (!i.userNumber.isNullOrEmpty()) token.stuId = i.userNumber
                        if (!i.facultyName.isNullOrEmpty()) token.school = i.facultyName
                        if (!i.majorName.isNullOrEmpty()) token.major = i.majorName
                        if (!i.gradeName.isNullOrEmpty()) token.grade = i.gradeName
                        if (!i.userId.isNullOrEmpty()) token.id = i.userId
                        if (i.loginPattern == "student-login") token.stutype = EASToken.TYPE.UNDERGRAD
                    }
